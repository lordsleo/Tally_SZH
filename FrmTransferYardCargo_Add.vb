    ...
    Private Sub btnCommit_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnCommit.Click
        ...
        Try
            ...
            If i >= count Then
                ...
                sqlda.Update(ds)            '插入主表

                dv = Filldata(sqlMainID)    '取出主表当前插入行ID
                If dv.Count = 1 Then        '主表插入成功
                    mainConSubID = CType(dv.Item(0)(0), Integer)
                    For i = 0 To count - 1
                        Me.dsSubTable.Tables(0).Rows(i)(1) = mainConSubID   '写入主表当前插入行ID
                    Next

                    sqlSubTableda.Update(dsSubTable)    '更新子表
                    Me.Close()
                Else
                    MessageBox.Show("主表未能正常更新，存储失败！", "错误提示", MessageBoxButtons.OK, MessageBoxIcon.Error)
                End If
            Else
            End If
        Catch ex As SystemException
