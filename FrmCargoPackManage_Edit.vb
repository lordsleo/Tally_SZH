...
Public Class FrmCargoPackManage_Edit
...
#Region " Windows 窗体设计器生成的代码 "
    ...
    <System.Diagnostics.DebuggerStepThrough()> Private Sub InitializeComponent()
        ...
        Me.Name = "FrmCargoPackManage_Edit"
...
#End Region
    ...
    Private Sub FrmCargoPackManage_Edit_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        On Error GoTo Err
...
Err:
    ...
    Private Sub btSave_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btSave.Click
        ...
        "' and not ( datediff(mi, '" & dtpTimeEnd.Value & "' ,time_from) >= 0 or datediff(mi,time_to, '" & dtpTimeBegin.Value & "' ) >= 0 ) " & _
        " and PACK_MANAGE_ID <> '" & ID & "'"
        Try
            If Trim(txtTeam.Text) <> "" And Trim(txtLeader.Text) <> "" Then
                If DateDiff(DateInterval.Minute, dtpTimeBegin.Value, dtpTimeEnd.Value) > 0 Then
                    If SetNumValue("LEAD_AMOUNT", txtLeadAmount.Text, "领数") = False Then    '输入不合法，不能插入
                        Exit Sub
                    ...
                    ElseIf SetNumValue("MARGIN", txtMargin.Text, "差额") = False Then
                        Exit Sub
                    End If
                    ds.Tables(0).Rows(0).Item("TEAM") = Trim(txtTeam.Text)
                    ds.Tables(0).Rows(0).Item("BAG_LEADER") = Trim(txtLeader.Text)
                    ds.Tables(0).Rows(0).Item("PROVIDER") = Trim(txtProvider.Text)
                    ds.Tables(0).Rows(0).Item("TIME_FROM") = dtpTimeBegin.Value
                    ds.Tables(0).Rows(0).Item("TIME_TO") = dtpTimeEnd.Value
                    ds.Tables(0).Rows(0).Item("TIME_LEAD_RECEDE") = dtpTimeLead.Value
                    ds.Tables(0).Rows(0).Item("GOODS_CODE") = cbGoodsCode.SelectedValue

                    ds.Tables(0).Rows(0).Item("DEPT_CODE") = FrmCargoPackManage.strCodeDept
                    ...
                    If MessageBox.Show("确认要修改吗？", "警告提示", MessageBoxButtons.OKCancel, MessageBoxIcon.Asterisk, MessageBoxDefaultButton.Button2) = DialogResult.OK Then
                        If Filldata(sqlExist).Count = 0 Then
                            '记录操作日志
                            Call OperateHistory(G_DeptName, Now, G_User, "修改", "CARGO_PACK_MANAGE", Trim(txtID.Text), "PACK_MANAGE_ID")
