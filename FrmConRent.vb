...
#Region " Windows 窗体设计器生成的代码 "
    ...
    <System.Diagnostics.DebuggerStepThrough()> Private Sub InitializeComponent()
        ...
        Dim resources As System.Resources.ResourceManager = New System.Resources.ResourceManager(GetType(FrmConRent))
...
#End Region
    ...
    Private Sub FrmConRent_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
    ...
    Private Sub C1DBGV_DoubleClick(ByVal sender As Object, ByVal e As System.EventArgs) Handles C1DBGV.DoubleClick
        ...
        If dsShip.Tables(0).DefaultView.Count > 0 Then
            If Not IsDBNull(dwship.Item(0)("AGENT")) Then
                strShipAgent = dwship.Item(0)("AGENT")
            End If
        ...
        Call preview("WL4_11") '*****************权限设计 '*************要修改模块号
...
Err:
    ...
    '    Try
    '        '不显示的列名在最前面
    '        strsql = "select " & SelStr & " *  from View_ConTallyDetail  where " & Sql '******要修改

    '        Me.C1DBG.DataSource = Getdata(strsql, ds)  ' class dbcontrol getdata fuction
    '        ColNum = 2 '不显示的列数 '******要修改
    '        Table_Name = "View_ConTallyDetail" '操作的数据库表名 ''******要修改
    ...
    Private Sub myDataGrid(ByVal SelStr As String, ByVal Sql As String)
        ...
        Dim i As Integer
        Try
            Dim sqlShipAgent As String = "SELECT CODE_SHIP_AGENT, SHIPAGENT_CHA, SHIPAGENT_ENG  FROM CODE_SHIP_AGENT ORDER BY CODE_SHIP_AGENT"
            Dim dvShipAgent As New DataView()
            dvShipAgent = Filldata(sqlShipAgent)
            Dim dvShipAgentEng As New DataView()
            dvShipAgentEng = Filldata(sqlShipAgent)

            Dim sqlContain As String = "SELECT CON_TALLY_DETAIL_ID, CONTAINER_NO,BAYNO,SIZE_CON,SEALNO,CODE_SHIP_AGENT, CODE_SHIP_AGENT as AgentCha, CODE_SHIP_AGENT as AgentEng FROM CON_TALLY_DETAIL WHERE CON_TALLY_LIST_ID IN (SELECT con_tally_list_id FROM con_tally_list WHERE ship_id = '" & Ship_ID & "')  and " & Sql
            ...
            'Me.C1DBG.Columns.Item("AgentCha").Caption = "船代公司"
            Me.C1DBG.Splits(0).DisplayColumns(0).Visible = False

            Me.C1DBG.Columns("AgentCha").ValueItems.Presentation = C1.Win.C1TrueDBGrid.PresentationEnum.ComboBox
            Me.C1DBG.Columns("AgentCha").ValueItems.Translate = True
            Me.C1DBG.Columns("AgentEng").ValueItems.Presentation = C1.Win.C1TrueDBGrid.PresentationEnum.ComboBox
            Me.C1DBG.Columns("AgentEng").ValueItems.Translate = True
            'Dim i As Short
            For i = 0 To dvShipAgent.Count - 1
                Dim VI As New C1.Win.C1TrueDBGrid.ValueItem()
                VI.DisplayValue = dvShipAgent.Item(i)(1)
                VI.Value = dvShipAgent.Item(i)(0)
                Me.C1DBG.Columns.Item("AgentCha").ValueItems.Values.Add(VI)

                Dim vItem As New C1.Win.C1TrueDBGrid.ValueItem()
                vItem.DisplayValue = dvShipAgentEng.Item(i)(2)
                vItem.Value = dvShipAgentEng.Item(i)(0)
                Me.C1DBG.Columns.Item("AgentEng").ValueItems.Values.Add(vItem)
            Next

            Table_Name = "CON_TALLY_DETAIL" '操作的数据库表名 ''******要修改
...
Err:
    ...
    Private Sub mnuEdit_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles mnuEdit.Click
        ...
        'End If
        Try
            If MessageBox.Show("确实要对所选记录的船代公司还原吗？", "操作提示", MessageBoxButtons.OKCancel, MessageBoxIcon.Information, MessageBoxDefaultButton.Button2) = DialogResult.OK Then
                Dim i As Short
                For i = 0 To Me.C1DBG.SelectedRows.Count - 1
                    Me.C1DBG.Row = Me.C1DBG.SelectedRows.Item(i)
                    Me.C1DBG.Columns("CODE_SHIP_AGENT").Value = Me.strShipAgent
                Next
                Me.C1DBGV.Focus()
                sqlda.Update(ds)
                Call myDataGrid(SelectStr, SqlStr)
