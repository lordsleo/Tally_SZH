...
#Region " Windows 窗体设计器生成的代码 "
    ...
    <System.Diagnostics.DebuggerStepThrough()> Private Sub InitializeComponent()
        ...
        Dim resources As System.ComponentModel.ComponentResourceManager = New System.ComponentModel.ComponentResourceManager(GetType(File_bak))
        ...
        Me.GB.Name = "GB"
        Me.GB.Size = New System.Drawing.Size(510, 397)
        Me.GB.TabIndex = 2
        Me.GB.TabStop = False
        '
        'Label
        ...
        Me.ToolBar.Size = New System.Drawing.Size(504, 41)
        ...
        Me.tbbtnAdd.Text = "下载"
        Me.tbbtnAdd.ToolTipText = "下载"
        '
        'tbbtnEdit
        '
        Me.tbbtnEdit.ImageIndex = 3
        Me.tbbtnEdit.Name = "tbbtnEdit"
        Me.tbbtnEdit.Text = "上传"
        Me.tbbtnEdit.ToolTipText = "上传"
        '
        'tbbtnDelete
        '
        ...
        Me.tbbtnPrint.Text = "设置"
        Me.tbbtnPrint.ToolTipText = "路径设置"
        '
        'tbbtnExport
        '
...
#End Region
    ...
    Dim downpath As String '设置下载文件存放目录
    Dim PathStr As String
    Dim FilePath As String
    Dim Save_FilePath As String


    Private Sub File_bak_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
    ...
    Private Sub ToolBarV_ButtonClick(ByVal sender As System.Object, ByVal e As System.Windows.Forms.ToolBarButtonClickEventArgs) Handles ToolBarV.ButtonClick
        Select Case Me.ToolBarV.Buttons.IndexOf(e.Button)
            ...
            Case 1
                'SqlStrShip = "where ((DEPT_CODE LIKE '" & G_DeptCode & "%' and SHIP_STATU in ('1','2','3')) OR (Ship_Statu in ('1') and len(ltrim(BerthNo))=0))  and code_goods<>'2' " '船舶可变的条件设计
                SqlStrShip = "where (DEPT_CODE LIKE '" & G_DeptCode & "%' and SHIP_STATU in ('2','3') and code_goods<>'2') "
            ...
            Case 3
                ...
                ElseIf Flag = 2 Then
                    ...
                    If G_User_Serial <> "wlgch" Then
                        SqlStrShip = "where (DEPT_CODE LIKE '" & G_DeptCode & "%' and SHIP_STATU in ('2','3') and code_goods<>'2') "
                    Else
                        SqlStrShip = "where (DEPT_CODE LIKE '" & G_DeptCode & "%' and SHIP_STATU in ('2','3','4') and code_goods<>'2') "
                    End If
                    '可能要修改
...
Err:
    ...
    Private Sub myDataGrid()
        ...
        Dim i As Integer
        Try
            strsql = "SELECT id,file_name,upload_date,modifi_date,upload_file,file_size,user_name,downloadtimes,download_name FROM File_bak where SHIP_ID=" & Ship_ID & " order by upload_date desc"
            dv = Getdata(strsql, ds)
            Me.C1DBG.DataSource = dv
            Me.C1DBG.Splits(0).DisplayColumns.Item(0).Visible = False
            Me.C1DBG.Splits(0).DisplayColumns.Item(3).Visible = False
            Me.C1DBG.Splits(0).DisplayColumns.Item(6).Visible = False
            Me.C1DBG.Splits(0).DisplayColumns.Item(7).Visible = False
            Me.C1DBG.Columns.Item(1).Caption = "文件名"
            Me.C1DBG.Columns.Item(2).Caption = "文件上传时间"
            Me.C1DBG.Columns.Item(3).Caption = "最后下载时间"
            'Me.C1DBG.Columns.Item(3).Caption = "船图文件"
            Me.C1DBG.Columns.Item(4).Caption = "文件大小(K)"
            Me.C1DBG.Columns.Item(5).Caption = "上传人员"
            Me.C1DBG.Columns.Item(6).Caption = "下载次数"
            Me.C1DBG.Columns.Item(7).Caption = "最近下载人员"

            Me.C1DBG.Columns.Item(2).NumberFormat = "yy/MM/dd HH:mm"
            Me.C1DBG.Columns.Item(3).NumberFormat = "yy/MM/dd HH:mm"
...
Err:
    ...
    Private Sub mnuDelete_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles mnuDelete.Click
        ...
        If ds.Tables(0).Rows.Count > 0 Then
            If G_User_Serial <> "wlgch" Then
                Return
            End If
            'Dim FrmDelete As New File_bak_delete   '***********要修改
            'FrmDelete.ShowDialog()
            Try
                i = CType(Me.C1DBG.Columns.Item("id").Text.ToString, Integer)
                If MsgBox("是否删除当前文件？", MsgBoxStyle.YesNoCancel, "提示") = MsgBoxResult.Yes Then
                    strsql = "delete FROM File_bak where ID=" & i
                    ExecSql(strsql)
                End If
                Call myDataGrid()
            Catch ex As Exception
    ...
    Private Sub mnuAdd_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles mnuAdd.Click
        ...
        Dim strsql As String
        Try
            'wjbc.AddExtension = True
            wjbc.Title = Me.Text + "——文件保存"
            wjbc.Filter = "Excel文件(*.xls)|*.xls|PDF文件(*.pdf)|*.pdf|All files (*.*)|*.*"
            wjbc.InitialDirectory = Save_FilePath
            'wjbc.FilterIndex = 2
            wjbc.FileName = dv(Me.C1DBG.Row)("file_name")

            If wjbc.ShowDialog() = DialogResult.OK Then
                Dim MyData() As Byte
                MyData = dv(Me.C1DBG.Row)("upload_file")
                Dim K As Long
                K = UBound(MyData)
                ...
                strsql = "UPDATE File_bak set modifi_date='" & Now.ToString & "',download_name='" & G_User & "', downloadtimes=downloadtimes+1 where ID=" & Me.C1DBG(Me.C1DBG.Row, "id")
                ExecSql(strsql)
    ...
    Private Sub mnuQuery_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles mnuQuery.Click
        'If ds.Tables(0).Rows.Count > 0 Then
        '    Dim FrmQuery As New File_bak_query   '***********要修改
    ...
    Private Sub ExportExcel() '导出Excel        
        ''Call MakeExcel()
        'Dim frmprint As New File_bak_print
        'frmprint.ShowDialog()
    End Sub
