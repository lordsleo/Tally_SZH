...
Public Class FrmREPORT_TALLY_QUALITY_ADD
...
#Region " Windows 窗体设计器生成的代码 "
    ...
    <System.Diagnostics.DebuggerStepThrough()> Private Sub InitializeComponent()
        ...
        Me.Name = "FrmREPORT_TALLY_QUALITY_ADD"
...
#End Region
    ...
    Private Sub FrmREPORT_TALLY_QUALITY_Edit_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        sqlstr = "select * from REPORT_TALLY_QUALITY where 1>2"
    ...
    Private Sub btSave_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btSave.Click
        Try
            If MsgBox("你确认要增加数据吗？", MsgBoxStyle.YesNo + vbExclamation, "提示") = MsgBoxResult.Yes Then
                ...
                sqldv = "SELECT * FROM REPORT_TALLY_QUALITY WHERE Code_ITEM='" & CmbCODE_TALLY_QUALITY.SelectedValue & "' and datediff(m,YearMonth,'" & DTPYEARMONTH.Value & "')=0 "
                ...
                row("Code_ITEM") = CmbCODE_TALLY_QUALITY.SelectedValue

                If Len(Trim(TxtITEMA.Text)) > 0 Then
                    If IsNumeric(Trim(TxtITEMA.Text)) Then
                        row("ITEMA") = Trim(TxtITEMA.Text)
                    Else
                ...
                Else
                    row("ITEMB") = 0
                End If
                row("ITEMC") = Trim(TxtITEMA.Text) / Trim(TxtITEMB.Text)


                If Len(Trim(TxtITEMD.Text)) > 0 Then
                    If IsNumeric(Trim(TxtITEMD.Text)) Then
                        row("ITEMD") = Trim(TxtITEMD.Text)
                    Else
                ...
                Else
                    row("ITEME") = 0
                End If
                row("ITEMF") = Trim(TxtITEMD.Text) / Trim(TxtITEME.Text)


                If Len(Trim(TxtITEMG.Text)) > 0 Then
                    If IsNumeric(Trim(TxtITEMG.Text)) Then
                        row("ITEMG") = Trim(TxtITEMG.Text)
                    Else
                ...
                Else
                    row("ITEMH") = 0
                End If
                row("ITEMI") = Trim(TxtITEMG.Text) / Trim(TxtITEMH.Text)


                If Len(Trim(TxtITEMJ.Text)) > 0 Then
                    If IsNumeric(Trim(TxtITEMJ.Text)) Then
                        row("ITEMJ") = Trim(TxtITEMJ.Text)
                    Else
                ...
                Else
                    row("ITEMK") = 0
                End If
                row("ITEML") = Trim(TxtITEMJ.Text) / Trim(TxtITEMK.Text)
