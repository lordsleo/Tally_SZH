...
Public Class FrmShipAgent_Edit
...
#Region " Windows 窗体设计器生成的代码 "
    ...
    <System.Diagnostics.DebuggerStepThrough()> Private Sub InitializeComponent()
        ...
        'btQuit
        '
        Me.btQuit.Location = New System.Drawing.Point(241, 176)
        ...
        'btResume
        '
        Me.btResume.Location = New System.Drawing.Point(173, 176)
        Me.btResume.Name = "btResume"
        Me.btResume.Size = New System.Drawing.Size(65, 24)
        Me.btResume.TabIndex = 133
        Me.btResume.Text = "确认"
        '
        'txtTelephone
        '
        ...
        'txtShort
        '
        Me.txtShort.Location = New System.Drawing.Point(64, 146)
        ...
        'ckShip
        '
        Me.ckShip.Location = New System.Drawing.Point(246, 146)
        Me.ckShip.Name = "ckShip"
        Me.ckShip.Size = New System.Drawing.Size(74, 24)
        Me.ckShip.TabIndex = 153
        Me.ckShip.Text = "船代标志"
        '
        'FrmShipAgent_Edit
        '
    ...
    Private Sub btResume_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btResume.Click
        Try
            If MsgBox("你确认要修改吗？", MsgBoxStyle.YesNo, "提示") = MsgBoxResult.Yes Then
                Call OperateHistory(G_DeptName, Now, G_User, "修改", "Code_Ship_Agent", Trim(txtID.Text), "Ship_Agent_ID")


                ds.Tables(0).Rows(0).Item("CODE_SHIP_AGENT") = Trim(txtCode.Text)
                ds.Tables(0).Rows(0).Item("SHIPAGENT_CHA") = Trim(txtCHA.Text)
                ds.Tables(0).Rows(0).Item("SHIPAGENT_ENG") = Trim(txtENG.Text)
                ds.Tables(0).Rows(0).Item("ADDRESS") = Trim(txtAddress.Text)
                ds.Tables(0).Rows(0).Item("TELEPHONE") = Trim(txtTelephone.Text)
                ds.Tables(0).Rows(0).Item("FAX") = Trim(txtFax.Text)
                ds.Tables(0).Rows(0).Item("E_MAIL") = Trim(txtEMail.Text)
                ds.Tables(0).Rows(0).Item("BANK_OPEN") = Trim(txtBank.Text)
                ds.Tables(0).Rows(0).Item("ACCOUNT") = Trim(txtAccount.Text)
                ds.Tables(0).Rows(0).Item("LINKMAN") = Trim(txtLinkMan.Text)
                ds.Tables(0).Rows(0).Item("SHIPAGENT_SHORT") = Trim(txtShort.Text)
                If ckShip.Checked = True Then
