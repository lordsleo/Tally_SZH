...
Public Class FrmCARGO_DAILY_REPORT_QUERY
...
#Region " Windows 窗体设计器生成的代码 "
    ...
    <System.Diagnostics.DebuggerStepThrough()> Private Sub InitializeComponent()
        Dim resources As System.Resources.ResourceManager = New System.Resources.ResourceManager(GetType(FrmCARGO_DAILY_REPORT_QUERY))
        ...
        Me.DTPdate.Location = New System.Drawing.Point(76, 19)
        ...
        'CombWORK_TIME
        '
        Me.CombWORK_TIME.Location = New System.Drawing.Point(256, 20)
        ...
        'CombCHIEF_TALLY
        '
        Me.CombCHIEF_TALLY.Location = New System.Drawing.Point(448, 20)
        ...
        'BTQUIT
        '
        Me.BTQUIT.Location = New System.Drawing.Point(305, 64)
        ...
        'BTOK
        '
        Me.BTOK.Location = New System.Drawing.Point(225, 64)
        Me.BTOK.Name = "BTOK"
        ...
        'RBchina
        '
        Me.RBchina.Location = New System.Drawing.Point(40, 248)
        ...
        'RBeng
        '
        Me.RBeng.Location = New System.Drawing.Point(128, 248)
        Me.RBeng.Name = "RBeng"
        ...
        Me.C1DBG.Location = New System.Drawing.Point(8, 100)
        Me.C1DBG.Name = "C1DBG"
        Me.C1DBG.PreviewInfo.Location = New System.Drawing.Point(0, 0)
        Me.C1DBG.PreviewInfo.Size = New System.Drawing.Size(0, 0)
        Me.C1DBG.PreviewInfo.ZoomFactor = 75
        Me.C1DBG.Size = New System.Drawing.Size(576, 132)
        ...
        " HorizontalScrollGroup=""1""><Height>128</Height><CaptionStyle parent=""Style2"" me=" & _
        ...
        "tyle1"" /><ClientRect>0, 0, 572, 128</ClientRect><BorderSide>0</BorderSide><Borde" & _
        ...
        "tArea>0, 0, 572, 128</ClientArea><PrintPageHeaderStyle parent="""" me=""Style14"" />" & _
        ...
        'CheckBox1
        '
        Me.CheckBox1.Location = New System.Drawing.Point(72, 64)
        ...
        Me.Name = "FrmCARGO_DAILY_REPORT_QUERY"
...
#End Region
    ...
    Private Sub FrmCARGO_DAILY_REPORT_QUERY_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        Me.Height = 130
    ...
    Private Sub BTOK_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BTOK.Click
        Try
            ...
            If RBchina.Checked = True Then
                dsdaily.Reset()
                dw = Getdata("exec sp_daily_report '" & Ship_ID & "','" & DTPdate.Value & "','" & CombWORK_TIME.SelectedValue & "' ", dsdaily)

                REMARKS = ""
                If dsdaily.Tables(1).Rows.Count > 0 Then
                    If dsdaily.Tables(1).Rows(0).Item("shifting") <> 0 Then
                        REMARKS = REMARKS & "出舱翻舱 " & dsdaily.Tables(1).Rows(0).Item("shifting") & " 件；"
                    End If
                    If dsdaily.Tables(1).Rows(0).Item("HOLIDAY") <> 0 Then
                        REMARKS = REMARKS & "节假日 " & dsdaily.Tables(1).Rows(0).Item("HOLIDAY") & " 件；"
                    End If
                    If dsdaily.Tables(1).Rows(0).Item("NIGHT") <> 0 Then
                        REMARKS = REMARKS & "夜班 " & dsdaily.Tables(1).Rows(0).Item("NIGHT") & " 件；"
                    End If
                    If dsdaily.Tables(1).Rows(0).Item("NON_CARGOHOLD") <> 0 Then
                        REMARKS = REMARKS & "非一般货舱 " & dsdaily.Tables(1).Rows(0).Item("NON_CARGOHOLD") & " 件；"
                    End If
                    If dsdaily.Tables(1).Rows(0).Item("MARK_ASSORTING") <> 0 Then
                        REMARKS = REMARKS & "分标志 " & dsdaily.Tables(1).Rows(0).Item("MARK_ASSORTING") & " 件；"
                    End If
                    If dsdaily.Tables(1).Rows(0).Item("STANDBY_time") <> 0 Then
                        REMARKS = REMARKS & "待时 " & dsdaily.Tables(1).Rows(0).Item("STANDBY_time") & " 件；"
                    End If
                End If
                OKQUERY = 1
                trade = "1"
            ...
            If RBeng.Checked = True Then
                rep1 = "0"
                rep2 = "0"
                st = "select id,gbno from report_his1 where ship_id='" & Ship_ID & "' and datediff(dd,workdate,'" & DTPdate.Value & "')=0 and worktime='" & CombWORK_TIME.SelectedValue & "' AND other='0' "
                Getdata(st, dstmp)
                If dstmp.Tables(0).Rows.Count > 0 Then
                    ...
                    End If
                Else
                    MsgBox("符合条件的日报不存在，请先处理！", MsgBoxStyle.OKOnly, "提示")
                    Me.Close()
                    'Exit Sub

                    'ExecSql("exec sp_daily_report_eng_gl '" & Ship_ID & "','" & DTPdate.Value & "','" & CombWORK_TIME.SelectedValue & "' ")
                ...
                '    st = "select id from report_his1 where ship_id='" & Ship_ID & "' and datediff(dd,workdate,'" & DTPdate.Value & "')=1 and worktime='02' AND other='0' "
                'Else
                '    st = "select id from report_his1 where ship_id='" & Ship_ID & "' and datediff(dd,workdate,'" & DTPdate.Value & "')=0 and worktime='01' AND other='0' "
                'End If
                'st = "select id from report_his1 where ship_id='" & Ship_ID & "' and datediff(dd,workdate,'" & DTPdate.Value & "')=0 and worktime='" & CombWORK_TIME.SelectedValue & "' "
                st = "select id from report_his1 where ship_id='" & Ship_ID & "' and gbno='" & lastgb & "' and other='0' "
                Getdata(st, dstemp)
                'If dstmp.Tables(0).Rows.Count > 0 Then
                '    rep1 = dstmp.Tables(0).Rows(0)(0)
                'End If
                ...
                If dstmp.Tables(0).Rows(0)(0) > 6 Then
                    dw = Getdata("exec sp_daily_report_eng_gl_tol_new '" & rep1 & "','" & rep2 & "' ", dsdaily)
                Else
                    dw = Getdata("exec sp_daily_report_eng_gl_tol '" & rep1 & "','" & rep2 & "' ", dsdaily)
                End If
