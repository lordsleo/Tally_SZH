...
#Region " Windows 窗体设计器生成的代码 "
    ...
    <System.Diagnostics.DebuggerStepThrough()> Private Sub InitializeComponent()
        ...
        Dim resources As System.ComponentModel.ComponentResourceManager = New System.ComponentModel.ComponentResourceManager(GetType(FrmSShip))
...
#End Region
    ...
    '*********2.2 '***************要修改
    Private Sub FrmSShip_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        Me.WindowState = FormWindowState.Maximized
        Call preview("WL2_2") '*****************权限设计  '*************要修改模块号
        Me.Text = Me.Text & "_" & G_DeptName
        FirmStr = " Order by Seq,Dept_Code,ID DESC  " '设计初始条件  在检索后不改变的条件  '***************要修改 或 2>1
        DynaStr = " DEPT_CODE LIKE '" & G_DeptCode & "%' and SHIP_STATU in ('0','1','2','3') OR (BerthNo='' or DEPT_CODE='') "
        SelectStr = " "
    ...
    Private Sub myDataGrid(ByVal SelStr As String, ByVal Sql As String)
        ...
        Try
            '不显示的列名在最前面
            strsql = "select " & SelStr & " * from VIEW_Ship where " & Sql
            ...
            Next
            Me.C1DBG.Columns("TONS").NumberFormat = "N"
            Me.C1DBG.Columns.Item("worktime_begin").NumberFormat = "yy/MM/dd HH:mm"
            Me.C1DBG.Columns.Item("worktime_end").NumberFormat = "yy/MM/dd HH:mm"
    ...
    Private Sub SetColumnWidth()
        Try
            ...
            For j = 0 To Me.C1DBG.Columns.Count - 1
                ...
                End If
            Next
            Me.C1DBG.Splits(0).DisplayColumns.Item("DEPT_NAME").Width = 40
            ...
            Me.C1DBG.Splits(0).DisplayColumns.Item("Trade").Style.HorizontalAlignment = C1.Win.C1TrueDBGrid.AlignHorzEnum.Center
            Me.C1DBG.Splits(0).DisplayColumns.Item("InOutPort_Name").Style.HorizontalAlignment = C1.Win.C1TrueDBGrid.AlignHorzEnum.Center
            Me.C1DBG.Splits(0).DisplayColumns.Item("SHIP_TYPE").Style.HorizontalAlignment = C1.Win.C1TrueDBGrid.AlignHorzEnum.Center
...
Err:
    ...
    Private Sub mnuFind_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles mnuFind.Click
        ...
        If Flag = 0 Then
            SelectStr = ""
            Call myDataGrid(SelectStr, "(DEPT_CODE LIKE '" & G_DeptCode & "%' or DEPT_CODE is null) and (" & Sql_Find & ")  " & FirmStr)  '可能要修改
            SqlStr = "DEPT_CODE LIKE '" & G_DeptCode & "%' and (" & Sql_Find & ") " & FirmStr '可能要修改
        ElseIf Flag = 2 Then
            SelectStr = ""
            DynaStr = "((DEPT_CODE LIKE '" & G_DeptCode & "%' or DEPT_CODE is null ) and SHIP_STATU in ('0','1','2','3','4')) OR (Ship_Statu in ('1') and len(ltrim(BerthNo))=0) "
    ...
    Private Sub mnuDelete_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles mnuDelete.Click
        If ds.Tables(0).Rows.Count > 0 Then
            ...
            Dim FrmDelete As New FrmSShip_Delete() '***********要修改
            ID = CType(Me.C1DBG.Columns.Item("ID").Text.ToString, Integer)
            sqldv = "SELECT SHIP_STATU FROM SSHIP WHERE SHIP_ID='" & ID & "'"
            dvc = Getdata(sqldv, dsc)
            If dsc.Tables(0).Rows(0).Item("SHIP_STATU").ToString() = "4" Then
                MsgBox("该状态的航次记录不能行删除！")
                Exit Sub
            Else
    ...
    Private Sub mnuEdit_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles mnuEdit.Click
        'If G_Part_Id = "14" Or G_Part_Id = "21" Or G_Part_Id = "28" Or G_Part_Id = "1" Then 'EDI信息员,值班主任
        If ds.Tables(0).Rows.Count > 0 Then
            Dim sqldc As String
            Dim sqldv As String
            Dim dvc As New DataView
            Dim dsc As New DataSet
            Dim FrmEdit As New FrmSShip_Edit() '***********要修改
            Dim Row As Integer
            ID = CType(Me.C1DBG.Columns.Item("ID").Text.ToString, Integer)
            Row = Me.C1DBG.Row
            sqldv = "SELECT SHIP_STATU FROM SSHIP WHERE SHIP_ID='" & ID & "'"
            dvc = Getdata(sqldv, dsc)
            If dsc.Tables(0).Rows(0).Item("SHIP_STATU").ToString() > "2" Then
                MsgBox("该状态的航次记录不能行编辑！")
                Exit Sub
            Else
                FrmEdit.ShowDialog()
            End If
    ...
    Private Sub mnuAdd_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles mnuAdd.Click
        Dim FrmAdd As New FrmSShip_Add()  '***********要修改
        Dim Row As Integer
        If ds.Tables(0).Rows.Count > 0 Then
            ...
            Row = Me.C1DBG.Row
            ID = CType(Me.C1DBG.Columns.Item("ID").Text.ToString, Integer)
            GL_ID = CType(Me.C1DBG.Columns.Item("V_ID").Text.ToString, Integer)
            'selectsql = "select * from SShip where V_ID=" & GL_ID & " and ship_statu in ('1','2','3')"
            'dw_tmp = Getdata(selectsql, ds_tmp)
            'If dw_tmp.Count > 0 Then
            '    '    GL_ID = -1
            '    '    MsgBox("此船已在港或已预报！", MsgBoxStyle.OKOnly + MsgBoxStyle.Exclamation, "提示")
            '    '    Exit Sub
            '    'ElseIf dw_tmp.Count = 1 Then
            '    GL_ID = CType(Me.C1DBG.Columns.Item("V_ID").Text.ToString, Integer)
            'End If
        Else
            ID = -1
    ...
    Private Sub C1DBG_FetchRowStyle(ByVal sender As Object, ByVal e As C1.Win.C1TrueDBGrid.FetchRowStyleEventArgs) Handles C1DBG.FetchRowStyle
        If Me.C1DBG.Columns("SHIP_STATU").CellText(e.Row) = "1" Then
            e.CellStyle.BackColor = Color.White
        ElseIf Me.C1DBG.Columns("SHIP_STATU").CellText(e.Row) = "2" Then
            e.CellStyle.BackColor = Color.PaleGreen
        ElseIf Me.C1DBG.Columns("SHIP_STATU").CellText(e.Row) = "3" Then
            e.CellStyle.BackColor = Color.LemonChiffon
        ElseIf Me.C1DBG.Columns("SHIP_STATU").CellText(e.Row) = "4" Then
            e.CellStyle.BackColor = Color.Pink
        ElseIf Me.C1DBG.Columns("SHIP_STATU").CellText(e.Row) = "5" Then
            e.CellStyle.BackColor = Color.Red
        ElseIf Me.C1DBG.Columns("SHIP_STATU").CellText(e.Row) = "0" Then
