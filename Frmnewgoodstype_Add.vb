...
Public Class Frmnewgoodstype_Add
...
Err:
    ...
    Private Sub btSave_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btSave.Click
        Try
            ...
            If Getdata("select * from newcodegoodstype where goods_type_code='" & Trim(txtCode.Text) & "'", dsSave).Count > 0 Then
                MsgBox("可能已存在此货类代码信息,请注意检查!")
                Exit Sub
            End If
            If Len(Trim(txtCode.Text)) > 0 And Len(Trim(txtCHA.Text)) > 0 Then
                ...
                row("goods_type_code") = Trim(txtCode.Text)
                row("goods_type_name") = Trim(txtCHA.Text)
                row("goods_type") = Trim(txtENG.Text)
                'row("Goods_Short") = Trim(txtShort.Text)
