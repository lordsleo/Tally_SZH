...
#End Region
    ...
    Private Sub Frm_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        DynaStr = " Code<>'0000' and Flag_Dept=1 " '设计其它初始条件    在检索后不需要的条件 ***************要修改 或 其它条件
        SelectStr = " " 'Top 30 "
    ...
    Private Sub myDataGrid(ByVal SelStr As String, ByVal Sql As String)
        ...
        Try
            '不显示的列名在最前面
            'strsql = "select " & SelStr & " ID, CODE, GOODS, Goods_Eng, Goods_Short, Code_Goods_Type, Code_Cargo_Type,Flag_Px  from CODEGoods where " & Sql '******要修改
            strsql = "select " & SelStr & " *  from VIEW_GOODS where " & Sql '******要修改
            Me.C1DBG.DataSource = Getdata(strsql, ds)  ' class dbcontrol getdata fuction
            ColNum = 3
            Table_Name = "VIEW_GOODS"
            Me.C1DBG.Columns.Item("code").Caption = "货名代码"
            For i = 0 To ColNum - 1
...
Err:
    ...
    Private Sub mnuDelete_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles mnuDelete.Click
        If ds.Tables(0).Rows.Count > 0 Then
            Dim FrmDelete As New FrmCargo_Delete   '***********要修改
    ...
    Private Sub mnuEdit_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles mnuEdit.Click
        If ds.Tables(0).Rows.Count > 0 Then
            Dim FrmEdit As New FrmCargo_Edit   '***********要修改
            Dim Row As Integer
            ID = CType(Me.C1DBG.Columns.Item("ID").Text.ToString, Integer)
            Row = Me.C1DBG.Row
            FrmEdit.ShowDialog()
    ...
    Private Sub mnuAdd_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles mnuAdd.Click
        Dim FrmAdd As New FrmCargo_Add   '***********要修改
        If ds.Tables(0).Rows.Count > 0 Then
            ID = CType(Me.C1DBG.Columns.Item("ID").Text.ToString, Integer)
        Else
            ID = -1
    ...
    Private Sub mnuQuery_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles mnuQuery.Click
        If ds.Tables(0).Rows.Count > 0 Then
            Dim FrmQuery As New FrmCargo_Query   '***********要修改
            ID = CType(Me.C1DBG.Columns.Item("ID").Text.ToString, Integer)
            FrmQuery.ShowDialog()
        End If
    End Sub
