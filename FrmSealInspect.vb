...
#Region " Windows 窗体设计器生成的代码 "
    ...
    <System.Diagnostics.DebuggerStepThrough()> Private Sub InitializeComponent()
        ...
        Dim resources As System.Resources.ResourceManager = New System.Resources.ResourceManager(GetType(FrmSealInspect))
        ...
        Me.C1DBG.Size = New System.Drawing.Size(502, 341)
...
#End Region
    ...
    Dim strSealBegin, strSealEnd As String
    ...
    Private Sub FrmSealInspect_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        Me.WindowState = FormWindowState.Maximized
        Call preview("WL9_4") '*****************权限设计 '*************要修改模块号
        Me.Text = Me.Text & "_" & G_DeptName
        'FirmStr = " Order by ID DESC"  '" Use_Comp Like '" & Trim(G_DeptCode) & "%' and Hide_Company Like '" & Trim(G_Company) & "%' order by ID DESC" 
        FirmStr = " ORDER BY SEAL_STATE, Dept_Code, SEALNO "
        If G_DeptCode = "26" Then
            DynaStr = " 2>1 "
        Else
            DynaStr = " DEPT_CODE LIKE '" & G_DeptCode & "%' " '设计其它初始条件    在检索后不需要的条件 ***************要修改 或 其它条件
        End If
        SelectStr = " Top 20 "
        SelectStr = "  "
    ...
    Private Sub myDataGrid(ByVal SelStr As String, ByVal Sql As String, Optional ByVal iLocal As Short = -1)
        Dim strsql As String
        Dim i As Integer
        Dim dv As DataView
        Try
            '不显示的列名在最前面
            strsql = "select " & SelStr & " * from View_SealRecord where " & Sql '******要修改
            dv = Getdata(strsql, ds)
            'Dim a, b As Short
            'a = 1
            'b = 2
            'dv.RowFilter = "substring(sealno, " & a & " , " & b & " )='bb'"
            If iLocal > -1 Then
                'MessageBox.Show(Len(strSealBegin) & iLocal & Len(strSealBegin) - iLocal + 1, "111", MessageBoxButtons.OK)
                'MessageBox.Show("substring(sealno, " & iLocal & " , " & Len(strSealBegin) - iLocal + 1 & " )>= '" & Mid(strSealBegin, iLocal) _
                '& "' and substring(sealno, " & iLocal & " , " & Len(strSealBegin) - iLocal + 1 & " )<= '" & Mid(strSealEnd, iLocal) & "'", "111", MessageBoxButtons.OK)

                dv.RowFilter = "substring(sealno, " & iLocal & " , " & Len(strSealBegin) - iLocal + 1 & " )>= '" & Mid(strSealBegin, iLocal) _
                       & "' and substring(sealno, " & iLocal & " , " & Len(strSealBegin) - iLocal + 1 & " )<= '" & Mid(strSealEnd, iLocal) & "'"

            End If

            Me.C1DBG.DataSource = dv ' class dbcontrol getdata fuction
            ColNum = 4 '不显示的列数 '******要修改
            Table_Name = "View_SealRecord" '操作的数据库表名 ''******要修改
            ...
            Me.C1DBG.Columns("SEAL_STATE").ValueItems.Presentation = C1.Win.C1TrueDBGrid.PresentationEnum.ComboBox
            Me.C1DBG.Columns("SEAL_STATE").ValueItems.Translate = True
            Dim vi1 As New C1.Win.C1TrueDBGrid.ValueItem()
            vi1.Value = "1"
            vi1.DisplayValue = "在库"
            Me.C1DBG.Columns.Item("SEAL_STATE").ValueItems.Values.Add(vi1)
            Dim vi2 As New C1.Win.C1TrueDBGrid.ValueItem()
            vi2.Value = "2"
            vi2.DisplayValue = "出库"
            Me.C1DBG.Columns.Item("SEAL_STATE").ValueItems.Values.Add(vi2)
            Dim vi3 As New C1.Win.C1TrueDBGrid.ValueItem()
            vi3.Value = "3"
            vi3.DisplayValue = "在用"
            Me.C1DBG.Columns.Item("SEAL_STATE").ValueItems.Values.Add(vi3)
            Dim vi4 As New C1.Win.C1TrueDBGrid.ValueItem()
            vi4.Value = "4"
            vi4.DisplayValue = "注销"
            Me.C1DBG.Columns.Item("SEAL_STATE").ValueItems.Values.Add(vi4)
...
Err:
    ...
    Private Sub mnuFind_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles mnuFind.Click
        Dim FrmCheck As New FrmSealInspect_Check()  '***********要修改
        If ds.Tables(0).Rows.Count > 0 Then
        ...
        FrmCheck.ShowDialog()
        Try
            SqlStr = FrmCheck.strCondition
            Dim iLocal As Short = FrmCheck.iLocal
            strSealBegin = FrmCheck.strSealNOBegin
            strSealEnd = FrmCheck.strSealNOEnd
            Call myDataGrid(SelectStr, SqlStr, iLocal)
    ...
    Private Sub mnuQuery_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles mnuQuery.Click
        If ds.Tables(0).Rows.Count > 0 Then
            Dim FrmQuery As New FrmSealInspect_Query() '***********要修改
            ID = CType(Me.C1DBG.Columns.Item("ID").Text.ToString, Integer)
            FrmQuery.ShowDialog()
        End If
    End Sub
    ...
    Private Sub C1DBG_FetchRowStyle(ByVal sender As Object, ByVal e As C1.Win.C1TrueDBGrid.FetchRowStyleEventArgs) Handles C1DBG.FetchRowStyle
        If Me.C1DBG.Columns("Seal_STATE").CellValue(e.Row) = "1" Then
            e.CellStyle.BackColor = System.Drawing.Color.LemonChiffon
        ElseIf Me.C1DBG.Columns("Seal_STATE").CellValue(e.Row) = "2" Then
            e.CellStyle.BackColor = System.Drawing.Color.White
        ElseIf Me.C1DBG.Columns("Seal_STATE").CellValue(e.Row) = "3" Then
            e.CellStyle.BackColor = System.Drawing.Color.PaleGreen
        ElseIf Me.C1DBG.Columns("Seal_STATE").CellValue(e.Row) = "4" Then
    ...
    'Dim ibegin, iend, y1, y2, h, count As Integer

    'Private Sub C1DBG_MouseDown(ByVal sender As Object, ByVal e As System.Windows.Forms.MouseEventArgs) Handles C1DBG.MouseDown
    '    If e.Button = MouseButtons.Left Then

    '        y1 = e.Y
    '        h = Me.C1DBG.RowHeight
    '        ibegin = Int((y1 - Me.C1DBG.CaptionHeight - 1) / (h + 1))
    '        Me.C1DBG.Row = ibegin
    '    End If
    'End Sub

    'Private Sub C1DBG_MouseUp(ByVal sender As Object, ByVal e As System.Windows.Forms.MouseEventArgs) Handles C1DBG.MouseUp
    '    If e.Button = MouseButtons.Left Then
    '        'MessageBox.Show(Me.C1DBG.CaptionHeight & " " & Me.C1DBG.RowHeight)
    '        MessageBox.Show(Me.C1DBG.Size.Height)
    '        y2 = e.Y
    '        iend = Int((y2 - Me.C1DBG.CaptionHeight - 1) / (h + 1))
    '        Me.C1DBG.Row = iend
    '        MessageBox.Show("begin= " & ibegin & "  y1= " & y1 & "  y2= " & y2 & "  count= " & count & "  end= " & iend)
    '    End If
    'End Sub
