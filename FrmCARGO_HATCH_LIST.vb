...
#Region " Windows 窗体设计器生成的代码 "
    ...
    <System.Diagnostics.DebuggerStepThrough()> Private Sub InitializeComponent()
        ...
        Dim resources As System.ComponentModel.ComponentResourceManager = New System.ComponentModel.ComponentResourceManager(GetType(FrmCARGO_HATCH_LIST))
        ...
        'ToolBar
        '
        Me.ToolBar.Buttons.AddRange(New System.Windows.Forms.ToolBarButton() {Me.tbbtnFind, Me.tbbtnQuery, Me.tbbtnSep1, Me.tbbtnAdd, Me.tbbtnEdit, Me.tbbtnDelete, Me.tbbtnSep2, Me.tbbtnPrint, Me.tbbtnINPUT, Me.tbbtnSep3, Me.tbbtnQuit})
        ...
        Me.tbbtnINPUT.Text = "录入"
        Me.tbbtnINPUT.ToolTipText = "录入"
        '
        'tbbtnSep3
        '
...
#End Region
    ...
    Private Sub Frm_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        SqlStrShip = "where ((DEPT_CODE LIKE '" & G_DeptCode & "%' and SHIP_STATU in ('2','3')) OR (Ship_Statu in ('1') and len(ltrim(BerthNo))=0)) and code_goods<>'2' "
        Call myDataGridShip(SqlStrShip & SortShip) '& inoutstr    '*****************调用航次信息
    ...
    Private Sub ToolBarV_ButtonClick(ByVal sender As System.Object, ByVal e As System.Windows.Forms.ToolBarButtonClickEventArgs) Handles ToolBarV.ButtonClick
        Select Case Me.ToolBarV.Buttons.IndexOf(e.Button)
            ...
            Case 1
                SqlStrShip = "where ((DEPT_CODE LIKE '" & G_DeptCode & "%' and SHIP_STATU in ('2','3'))  OR (Ship_Statu in ('1') and len(ltrim(BerthNo))=0))  and code_goods<>'2' " '船舶可变的条件设计
...
Err:
    ...
    '新增加模块，将新舱单主子表中数据合并后保存至原舱单表中
    Private Sub readcargolist()
        Dim strsql As String
        Dim sql_name, sql_in, sql_out As String
        Dim dscargo As New DataSet

        strsql = "select * from cargo_hatch_new where ship_id=" & Ship_ID & " (oldid=0 or oldid is null) "
    ...
    Private Sub myDataGrid(ByVal SelStr As String, ByVal Sql As String)
        ...
        Try
            '不显示的列名在最前面
            'strsql = "select " & SelStr & " * from VIEW_CARGO_HATCH_LIST_NEW where " & Sql '******要修改
            If Hatch_in = 1 Then
                strsql = "select " & SelStr & " Ship_ID, ID, unload_port, BLNO, GOODS_ENG, PACK_ENG as pack_cha, mark, AMOUNT_ORI, TONS_ORI, SIZE_MEASURE, AMOUNT, GROSS_WEIGHT, USER_NAME from VIEW_CARGO_HATCH_LIST_NEW where " & Sql & " order by unload_port, BLNO " '******要修改
            Else
                strsql = "select " & SelStr & " Ship_ID, ID, load_port, BLNO, GOODS_ENG, PACK_ENG as pack_cha, mark, AMOUNT_ORI, TONS_ORI, SIZE_MEASURE, AMOUNT, GROSS_WEIGHT, USER_NAME from VIEW_CARGO_HATCH_LIST_NEW where " & Sql & " order by load_port, BLNO " '******要修改"
            End If
            ...
            ColNum = 2 '不显示的列数 '******要修改
            Table_Name = "view_cargo_hatch_list" '操作的数据库表名 ''******要修改
            ...
            'Me.C1DBG.Columns.Item("TONS_ORI").Caption = "原配吨数"
...
Err:
    ...
    Private Sub mnuEdit_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles mnuEdit.Click
        If ds.Tables(0).Rows.Count > 0 Then
            Dim FrmEdit As New FrmCargo_hatch_list_edit   '***********要修改
    ...
    Private Sub btResure_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btResure.Click
        SqlStrShip = "where (((DEPT_CODE LIKE '" & G_DeptCode & "%' and SHIP_STATU in ('1','2','3')) ) and Voyage like '" & Trim(txtVoyage.Text) & "%' and (ENG_VESSEL Like '" & Trim(txtShipName.Text) & "%' OR CHI_VESSEL like '" & Trim(txtShipName.Text) & "%' ))  and code_goods<>'2' "
        '可能要修改
